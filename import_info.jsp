...
<%
	...
	RecList recListName=null;
	String select_name="";
	String sql= "select * from enter where 1=1 ";
	String search_content="";
	if(d!=null && (!tt.equals(""))){


		if(Integer.parseInt(d)==-1){
			sql=sql+"and e_id like ? ";
		}

		else if(Integer.parseInt(d)==0){
			sql=sql+"and e_num like ? ";
		}

		else if(Integer.parseInt(d)==1){
			sql=sql+"and e_date like ? ";
		}		
		else if(Integer.parseInt(d)==2){
			sql=sql+"and e_supply like ? ";
		}	
		else{
...
<button style="right:10px; margin-left: 5px; background: rgba(85, 63, 122, 0.9);padding: 2px 12px;color:#fff;font-weight:300;bolid:0;"  id="query-emp-id" type="submit"data-href="import_info.jsp" class="btn dq-click" name="button">搜索</button>
...
<%-- 搜索部分 --%>
    ...
    <ul  style="margin:10px;"  class="nav nav-tabs">
      ...
      <li class="active"><a href="#import_info">入库信息</a></li>
      <li><a href="outport_info.jsp" class="dq-click">出库信息</a></li>
    </ul>

      <div class="tab-pane active" id="import_info">
      <%-- 基本信息表 --%>
...
<table id="table" style="margin:10px;" class="table table-striped table-hover table-condensed">
	<thead>
		<tr>
		<th>货物编号</th><th>货物名称</th><th onclick="paixu(2)">入库数量</th><th onclick="func(3)">入库时间</th><th>供应商</th>
		</tr>
	</thead>
	<tbody id="t1">
<%
if(recList!=null){
	for(int i=0;i<recList.size();i++){
	    ...
	    String eids = dep.get("e_id").toString();
		
		Rec deps = MyDao.queryOne("select * from pro_infor where pro_id=?", eids);
		
%>
		<tr>

			<td><% out.print(dep.get("e_id")); %></td>
			<td><% out.print(deps.get("pro_name")); %></td>
			<td><% out.print(dep.get("e_num")); %></td>
		    <td><% out.print(dep.get("e_date")); %></td>
		    <td><% out.print(dep.get("e_supply")); %></td>
